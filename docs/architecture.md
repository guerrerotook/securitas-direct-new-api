...
- **Lock command verification** (`_change_lock_mode` → `_poll_lock_until`): the backend acks a lock/unlock before the device physically actuates (~6s to start + ~4.5s to complete; see PR #413), so a single immediate read races ahead of the lock. We snapshot the pre-command `statusTimestamp`, then re-read up to `LOCK_VERIFY_ATTEMPTS` times (`LOCK_VERIFY_DELAY` apart) and confirm success only on `lockStatus == target` **with a fresh timestamp** (`_ts_is_newer`) — a stale target read does not confirm. The fresh-ts gate makes success fast (early-return) but can't shorten the *failure* decision (a still-completing lock and a failed one both read as pre-command state), so the window must cover the worst-case actuation (currently ~18s, past #413's validated 15s; tune from the per-attempt `statusTimestamp` debug logs).
