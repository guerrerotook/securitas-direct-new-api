...
**Discovery:** Cameras are discovered in the background task. `get_camera_devices()` returns devices of type `"QR"` (Italy and some regions), `"YR"` (PIR cameras, Spain), `"XR"` (photo cameras, first seen on a Spanish SDVFAST panel in #616; deviceType 106 assumed from YR, not confirmed), `"YP"` (perimetral exterior, deviceType 103), or `"QP"` (perimetral exterior, deviceType 107). For each device a `VerisureCamera` + `VerisureCameraFull` + `VerisureCaptureButton` are created using stored `async_add_entities` callbacks. The buttons are constructed with a `camera_entity=<thumbnail_entity>` reference so their deprecated `async_press` can delegate directly to `camera_entity.async_manual_capture()` instead of doing a runtime entity-id lookup. Devices with `isActive: null` are treated as active (only `isActive: False` is filtered out). Some installations return `zoneId: null` for every device (seen on YR and XR); zone_id then falls back to the type plus the code zero-padded to two digits (e.g. `XR01`), or to the device `id` when the code is missing or not numeric.
