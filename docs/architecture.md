...
- **Lock management** — `get_lock_modes()` discovers locks (cached with TTL), `change_lock_mode()` performs lock/unlock via queue, `get_smart_lock_config()` fetches per-lock configuration and features.
...
Lock and unlock operations use `change_lock_mode(lock=True/False)` which follows the same polling pattern as arm/disarm. The lock poll loop retries on `alarm-manager.error_no_response_to_request` (the panel hasn't heard back from the lock yet) rather than aborting — this is a transient condition. After the command is acknowledged, `change_lock_mode()` waits at least `_LOCK_CMD_MIN_WAIT` seconds for the lock to physically act, then invalidates the lock-modes cache so stale data is not returned by subsequent polls. The entity then immediately fetches the real lock status via `get_lock_state()` and updates accordingly, falling back to an optimistic state if the fresh poll returns UNKNOWN. While a lock command is in flight, `_operation_in_progress` suppresses periodic polling to prevent stale API responses from briefly overwriting the transitional state — the same pattern used by the alarm panel. Periodic background polling via `get_lock_current_mode()` resumes on the scan interval once the command completes.
...
The test suite has **830 tests** achieving **92% overall coverage**. Tests run on every PR via GitHub Actions with three parallel checks: Ruff lint/format, Pyright type checking, and pytest with a 90% coverage floor.
...
└── test_smart_lock.py       Smart lock config (flat params, features parsing), mode changes, status polling
```
...
| `hub.py` | 714 | `SecuritasHub` (central coordinator), `SecuritasDirectDevice` (device registry wrapper) |
...
| `lock.py` | 277 | Multi-lock entity with lock feature attributes |
...
| `securitas_direct_new_api/graphql_queries.py` | 237 | GraphQL query and mutation string constants |
...
| `securitas_direct_new_api/dataTypes.py` | 200 | Response dataclasses (including LockFeatures, LockAutolock, Camera, SStatus) |
