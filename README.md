...
The integration's HA domain is `securitas` for historical reasons, but every service, event, and card name is also registered under `verisure_owa.*` / `verisure_owa_*` — use that form when you write new automations. Old `securitas.*` automations keep working. See [`docs/FUTURE_MIGRATION_PLAN.md`](docs/FUTURE_MIGRATION_PLAN.md) for why this is unresolved.
...
**When to try it:** if you see that specific `DNS server returned answer with no data` error (or a similar IPv6/AAAA lookup failure), go to **Settings → Integrations → Verisure OWA → Configure**, expand the **Advanced** section, and turn on **Force IPv4-only connections**. The integration then looks the server up over IPv4 only, skipping the IPv6 query that was failing. The server has no IPv6 address, so you lose nothing by turning this on.

**Turn it on for every entry that uses the same Verisure account.** If you have more than one alarm (more than one entry) signed in with the same username, they all share a single connection to the Verisure server, and that connection is set up by whichever entry loads first. So switching only one of them on may change nothing: the shared connection is still the one the other entry created. Turn the setting on for each of those entries in turn, then restart Home Assistant so the shared connection is rebuilt with the new setting. With a single alarm, or with entries under different usernames, the change takes effect as soon as you save — the integration reloads itself.
