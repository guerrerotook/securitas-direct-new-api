...
Three fixes, and none needs anything from you: one for anyone whose alarm kept dropping to _unavailable_ with a DNS error, and two for how the integration sets up and tidies up after itself as installations are added, removed, switched off or fail to start, which matters most if you have more than one installation.
...
**A failed connection attempt left the shared sign-in behind ([#615](https://github.com/guerrerotook/securitas-direct-new-api/pull/615)).**  Installations on the same account share one sign-in, and the integration keeps track of how many are using it so it can let go of it when the last one goes. That tally was wrong in two ways. An installation that got as far as signing in but then failed — usually because the Verisure servers were unreachable — was counted, and counted again on every retry, so the tally could never fall back to zero and the integration never let go of the shared sign-in. And if you run more than one installation on one account, unloading one that had failed to start took the shared sign-in away from a working installation, leaving it to sign in again from scratch the next time it reloaded. That matters more than it sounds: the Verisure servers limit how often one address may sign in, and every sign-in replaces the token the integration keeps for the next restart. Each installation is now counted once however many times it retries, and unloading one that was never using the shared sign-in leaves it alone. Starting to set up an installation and then closing the setup dialog, or a setup that stopped early (for example because every installation was already set up), also left its sign-in behind for the next setup to pick up; it is now let go. Deleting an installation that had failed to start now also lets go of its share of the sign-in. And with two setup dialogs open for the same account, closing one no longer makes the other sign in again.

**Removing, switching off or adding installations now tidies up properly ([#619](https://github.com/guerrerotook/securitas-direct-new-api/pull/619)).**  A few rarer cases from the previous fix are now covered too. When nothing uses the integration any more, it stops loading its dashboard cards and removes the filters that tidy its log messages and the `verisure_owa.*` alternative names for its actions; three cases skipped that until Home Assistant restarted: switching off your last installations when one of them was still retrying its setup, two installations unloading at the same moment, and closing the setup dialog after removing your last installation while the dialog was open. An installation that started just as the integration stopped loading its dashboard cards was left with only one of its five cards loading until the next restart; it now waits and loads all five. And opening the setup dialog for an account while one of its installations was still signing in, for example just after Home Assistant started, signed that account in twice: the dialog now waits and uses the installation's sign-in, which spares one sign-in against the Verisure servers' limit on how often one address may sign in. Finally, while an installation waits for you to sign in to it again, the integration keeps loading its dashboard cards when you remove or switch off another installation.
