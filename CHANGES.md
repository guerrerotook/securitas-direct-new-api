...
- **Sub-panel arming modes the panel rejects are now disabled, persistently.** On an Italian SDVECU Interior sub-panel, pressing Armed Night triggers an `ARMNIGHT1` the panel rejects — there's no way to detect this in advance. Previously the error notification pointed users at the state-mapping UI (which sub-panels don't have) and the button came back on the next HA restart, so the same rejection happened again. The `CommandResolver` is now hydrated from a per-installation `unsupported_commands` list persisted in `entry.data`, and each 400-rejection appends to it. Sub-panels recompute `supported_features` against the resolver, so a rejected mode disappears from the card the moment it fails and stays gone across restarts. The notification now has a sub-panel-specific variant that explains the mode has been disabled, rather than pointing at the missing mappings UI.
