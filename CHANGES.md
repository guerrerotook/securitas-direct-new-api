...
- **Activity log unknown events.** Event type 820 ("Disattivazione Perimetrale", perimeter disarm — the disarm counterpart to the existing 821/823/824 perimeter-arm codes) is now mapped to `DISARMED` instead of `UNKNOWN`. Event type 14 ("Allarme Foto", a photo-detector alarm with an attached image) is now mapped to `ALARM` — same category as the generic type 13 alarm, with the image carried via the existing `img=1` flag on the event.
