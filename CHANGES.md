...
**Arming-exception sensors for every door and window contact ([#543](https://github.com/guerrerotook/securitas-direct-new-api/pull/543)).**  Each contact now gets an **Arming Exception** and a **Battery** binary sensor on its own device under the installation, alongside two installation-wide roll-ups that list every affected zone in their attributes.  This costs nothing: the panel has always sent this data with every status poll and the integration was simply discarding it, so nothing polls more often and no extra request is made.  The peripheral inventory needed to name the zones comes from the same `xSDeviceList` call camera discovery already makes, which is now fetched once and shared rather than twice.

They are deliberately **not** called door sensors, because they are not.  Measured against a live panel: with the alarm disarmed and a door standing wide open, the panel reports nothing; with the alarm armed and everything closed, it reports nothing; only arming *over* an open zone puts that zone in the list.  In other words the field describes zones that are exceptional for the current armed session, and there is no way to ask the Verisure API whether a door is open right now.  So **Arming Exception** turns on when you arm with that zone open or its battery flat, and off the rest of the time — which is a true statement in every alarm state, and is genuinely useful for catching that you armed with a window bypassed or that a sensor battery needs replacing.  A zone whose panel label cannot be matched to a known device still gets an entity under the label the panel uses, so nothing is silently dropped; two zones sharing a label cannot be told apart and are reported by the roll-ups only.
