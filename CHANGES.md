...
Three fixes, and none needs anything from you: one for anyone whose alarm kept dropping to _unavailable_ with a DNS error, and two for what the integration left behind after a setup that failed, was abandoned or was switched off, which matters most on accounts with more than one installation.
...
**Removing, switching off or adding installations now tidies up properly (#PR).**  A few rarer cases from the previous fix are now covered too. When nothing uses the integration any more, it removes its dashboard card resources, its log filters and its `verisure_owa.*` service aliases; three cases skipped that until Home Assistant restarted: switching off your last installations when one of them was still retrying its setup, two installations unloading at the same moment, and closing the setup dialog after removing your last installation while the dialog was open. An installation that started while the integration was removing its dashboard cards lost four of its five card resources until the next restart; it now waits and adds all five back. And opening the setup dialog for an account while one of its installations was still signing in, for example just after Home Assistant started, signed that account in twice: the dialog now waits and uses the installation's sign-in, which spares one sign-in against the Verisure servers' limit on how often one address may sign in.
