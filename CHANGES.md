...
Your door and window contacts, and the panel's own health, become binary sensors.  The local alarm PIN can now guard your smart locks as well as the alarm panel — and it is stored hashed rather than in plain text.  Three fixes besides: a long-standing bug that silently discarded your PIN and alarm mappings shortly after a fresh install, an integration left permanently down by a network blip during Home Assistant startup, and camera capture delivering nothing for long-idle cameras.

### Added

**Zone sensors for every door and window contact in your installation ([#543](https://github.com/guerrerotook/securitas-direct-new-api/pull/543)).**  Each contact now gets an **Open** and a **Battery** binary sensor on its own device under the installation, alongside two installation-wide roll-ups — **Zones Open** and **Zone Battery Low** — that list every affected zone in their attributes, so a dashboard can carry one card instead of twelve.  This costs nothing: the panel has always sent this data with every status poll and the integration was simply discarding it, so nothing polls more often and no extra request is made.  The peripheral inventory needed to name the zones comes from the same `xSDeviceList` call camera discovery already makes, which is now fetched once and shared rather than twice.

There is one behaviour worth understanding, because it looks like a bug and is not.  The panel reports *problems*, not *states*: it sends a list of the zones that are open or low on battery and says nothing at all about the ones that are fine.  An empty list is therefore ambiguous — it can mean "everything is closed", or it can mean this panel never reports zone problems in the first place.  Rather than guess, the sensors read **Unknown** until your panel has reported at least one real zone exception; from that point on they behave normally and an empty list correctly means all-clear.  For the same reason the per-zone entities do not appear the moment you upgrade.  Open a door once and the whole set materialises at once — every zone, not just the one that reported — and they persist across restarts from then on.  On an installation whose panel never populates the field, you get the two roll-ups sitting at Unknown and nothing else, which is the honest answer: reporting "closed" for a door nobody has any information about would be worse than reporting nothing.  A zone whose panel label cannot be matched to a known device still gets an entity under the label the panel uses, so nothing is silently dropped; two zones sharing a label cannot be told apart and are reported by the roll-ups only.

**Panel problem sensors for intrusion, power, comms and tampering ([#543](https://github.com/guerrerotook/securitas-direct-new-api/pull/543)).**  Four new binary sensors — **Alarm Triggered**, **Mains Power Cut**, **Communication Problem** and **Tamper** — surface conditions the panel's status endpoint does not expose at all, derived instead from the activity timeline you may already be polling.  Alarm Triggered clears when the panel reports the alarm resolved *or* when you disarm, because some panels never send the resolved signal and the sensor would otherwise stay on forever; disarming is the acknowledgement.  Tamper clears itself 24 hours after the last report, since the panel never says interference has ended and the alternatives are all worse — a sensor stuck on for life, or one cleared by the next routine morning disarm.

These four are created **only when Activity Log background polling is enabled** (Settings → Integrations → Verisure OWA → Configure).  They can only be as fresh as the activity log, and with polling off it refreshes solely when you open the activity card — a safety sensor that silently stops updating is worse than one that is not there, so the integration declines to create it.  Turning the setting off later leaves the entities in the registry as unavailable; turning it back on restores them with their `entity_id` and recorder history intact.
