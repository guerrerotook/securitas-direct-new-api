...
- **Clearing a mapping field actually clears it.** PR #463 swapped the "Not used" dropdown choice for "leave the field blank", but the select selector rejected empty values (so the field couldn't be cleared from the UI) and the options→data sync merged on top of stale values (so even an omitted key reverted to the previous setting). The mapping dropdown now includes an explicit "(Not used)" entry and the listener replaces options-managed keys in `entry.data` instead of merging — clearing a previously-set Vacation or Custom mapping now sticks.
