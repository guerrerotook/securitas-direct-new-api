...
async def async_unload_entry(hass: HomeAssistant, config_entry: ConfigEntry) -> bool:
    ...
    # half-disposed hub. Zone alias listeners go the same way: a coordinator update
    # mid-unload must not try to add entities to a dead platform.
    for unsub in entry_data.get("lock_config_retry_unsubs", []):
        unsub()
    for unsub in entry_data.get("zone_alias_unsubs", []):
        unsub()
