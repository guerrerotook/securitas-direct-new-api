...
// `securitas-*` tag-name aliases for the canonical `verisure-owa-*` cards.
// Kept registered indefinitely so user dashboards with
// `type: custom:securitas-alarm-card` continue to render — both names
// are equal-weight, no deprecation. The card picker only offers the
// `verisure-owa-*` form (see customCards.push block at the bottom of
// this file).
